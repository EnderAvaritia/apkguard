...
# --- 批量扫描汇总 / Batch scan summary ---

_RISK_CLASS = {
    "clean": "badge-ok",
    "suspicious": "badge-warn",
    "malicious": "badge-bad",
}
...
_DYN_STATUS_BADGE = {
    ...
    "degraded": "badge-warn",
    "skipped": "badge-muted",
    "not_executed": "badge-muted",
}


def _dyn_badge(status: str) -> str:
    cls = _DYN_STATUS_BADGE.get((status or "not_executed").lower(), "badge-muted")
    label = {
        "executed": "已执行 / Executed",
        "degraded": "降级 / Degraded",
        "skipped": "已跳过 / Skipped",
        "not_executed": "未执行 / Not executed",
    }.get((status or "not_executed").lower(), "未执行 / Not executed")
    return _badge(label, cls)


def render_scan_summary_html(
    entries: list[tuple[str, dict]], errors: list[str], scanned_dir: str
) -> str:
    """批量扫描汇总页（自包含 HTML）。

    ``entries`` 为 ``[(path, report.to_dict()), ...]``，已按分数降序。
    """
    counts = {"clean": 0, "suspicious": 0, "malicious": 0}
    for _, rep in entries:
        level = (rep.get("risk") or {}).get("risk_level")
        if level in counts:
            counts[level] += 1
    total = len(entries)
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    for path, rep in entries:
        ...
        level = risk.get("risk_level", "clean")
        label = risk.get("risk_label") or level
        score = risk.get("total_score", 0)
        app = rep.get("app_info") or {}
        dyn = rep.get("dynamic") or {}
        rows += (
            "<tr>"
            f'<td class="mono">{_esc(path)}</td>'
            f'<td>{_esc(app.get("package") or "-")}</td>'
            f'<td>{_badge(label, _RISK_CLASS.get(level, "badge-muted"))}</td>'
            f'<td><span class="score">{_esc(str(score))}</span></td>'
            f'<td>{len(rep.get("findings") or [])}</td>'
            f"{_dyn_badge(dyn.get('status'))}"
            f'<td class="mono">{_esc(str(rep.get("sha256", ""))[:12])}</td>'
            "</tr>"
        )
    table = f"""
<section class="card">
  <h2 class="card-title">样本清单 / Samples <span class="count">{total}</span></h2>
  <table class="findings-table">
    <thead><tr>
      <th>文件 / File</th><th>包名 / Package</th><th>风险 / Risk</th>
      <th>分 / Score</th><th>发现 / Findings</th><th>动态 / Dynamic</th><th>SHA256</th>
    </tr></thead>
    <tbody>{rows}</tbody>
  </table>
</section>"""

    c_clean = counts["clean"]
    c_susp = counts["suspicious"]
    c_mal = counts["malicious"]
    count_badges = (
        _badge("干净 / Clean: " + str(c_clean), "badge-ok")
        + " " + _badge("可疑 / Suspicious: " + str(c_susp), "badge-warn")
        + " " + _badge("恶意 / Malicious: " + str(c_mal), "badge-bad")
    )
    summary_card = f"""
<section class="card">
  <h2 class="card-title">统计 / Summary <span class="count">{total}</span></h2>
  <div class="status-line">{count_badges}</div>
  <p class="meta-note">扫描目录 / Dir: {_esc(scanned_dir)} · 生成时间 / Generated: {ts}</p>
</section>"""
    ...
    if errors:
        ...
        errors_card = f"""
<section class="card">
  <h2 class="card-title">失败 / Failed <span class="count">{len(errors)}</span></h2>
  <ul class="warn-list">{items}</ul>
</section>"""

    footer = f"""
<footer>
  <p>apkguard 批量扫描汇总 / Generated by apkguard</p>
  <p class="foot-sub">生成时间 / Generated at: {ts} · 文件数 / Files: {total}</p>
</footer>"""
    ...
    return (
        ...
        '<meta charset="utf-8">\n'
        '<meta name="viewport" content="width=device-width, initial-scale=1">\n'
        "<title>apkguard 批量扫描汇总 / Batch Scan Summary</title>\n"
        f"<style>{_CSS}</style>\n"
        "</head>\n<body>\n"
        f'<div class="wrap">{summary_card}{table}{errors_card}{footer}</div>\n'
        "</body>\n</html>"
    )


def write_scan_summary_html(
    entries: list[tuple[str, dict]], errors: list[str], scanned_dir: str, out_path: str | Path
) -> None:
    """批量扫描汇总落盘为 HTML（UTF-8）"""
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(render_scan_summary_html(entries, errors, scanned_dir))
